...
export default function PostulantesPage({
    ...
    const [localFilters, setLocalFilters] = useState<PostulanteFilters>({
    ...
    const [detailPostulante, setDetailPostulante] =
        useState<Postulante | null>(selectedPostulante ?? null);
    const [editPostulante, setEditPostulante] = useState<Postulante | null>(
        null,
    );
    ...
    const summary = useMemo(
        () => ({
            ...
            complete: postulantes.filter(
                (postulante) => postulante.documentacion_completa,
            ).length,
    ...
    return (
        <>
            ...
            <div className="space-y-6 p-4 md:p-6">
                ...
                <Card>
                    ...
                    <CardContent className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
                        ...
                        />
                        <Input
                            value={localFilters.colegio_procedencia}
                            onChange={(event) =>
                                setLocalFilters({
                                    ...localFilters,
                        ...
                        <Select
                            value={
                                localFilters.documentacion_completa || 'all'
                            }
                            onValueChange={(value) =>
                                setLocalFilters({
                                    ...localFilters,
                        ...
                        <Select
                            value={localFilters.estado_admision || 'all'}
                            onValueChange={(value) =>
                                setLocalFilters({
                                    ...localFilters,
                                    estado_admision:
                        ...
                        <Select
                            value={localFilters.id_carrera || 'all'}
                            onValueChange={(value) =>
                                setLocalFilters({
                                    ...localFilters,
                ...
                <Card>
                    ...
                    <div className="px-6 pb-6">
                        <PostulantesTable
                            ...
                            canUpdate={canUpdate}
                            onView={setDetailPostulante}
                            onEdit={setEditPostulante}
                        />
                    </div>
                </Card>
            </div>
            ...
            <Dialog
                open={detailPostulante !== null}
                onOpenChange={(open) => !open && setDetailPostulante(null)}
            >
                <DialogContent className="sm:max-w-3xl">
                    ...
                    {detailPostulante && (
                        <PostulanteDetail postulante={detailPostulante} />
                    )}
                </DialogContent>
            </Dialog>

            <Dialog
                open={editPostulante !== null}
                onOpenChange={(open) => !open && setEditPostulante(null)}
            >
                <DialogContent className="sm:max-w-3xl">
                    <DialogHeader>
                    ...
                    {editPostulante && (
                        <PostulanteForm
                            postulante={editPostulante}
                            carreras={carreras}
                            canSubmit={canUpdate}
                            onSuccess={() => setEditPostulante(null)}
                        />
                    )}
                </DialogContent>
            </Dialog>
