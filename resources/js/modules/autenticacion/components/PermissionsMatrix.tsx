...
import type { PermisosPorModulo } from '../types/access-control';
    ...
    return (
        <div className="max-h-80 space-y-4 overflow-y-auto rounded-md border p-4">
            {Object.entries(permisosPorModulo).map(([modulo, permisos]) => (
                <section key={modulo} className="space-y-3">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        {permisos.map((permiso) => (
                            <Label
                                ...
                                className="flex cursor-pointer items-start gap-3 rounded-md border p-3 text-sm hover:bg-slate-50"
                            >
                                <Checkbox
