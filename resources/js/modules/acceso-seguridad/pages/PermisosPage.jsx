...
export default function PermisosPage({ permisosPorModulo }) {
    ...
    const modules = useMemo(() => Object.entries(permisosPorModulo), [permisosPorModulo]);
    const openCreate = () => {
        setSelected(null);
        reset();
        setOpen(true);
    };
    const openEdit = (permiso) => {
        if (!canUpdate) {
            return;
        }
        setSelected(permiso);
    ...
    return (<>
            ...
            <div className="space-y-6 p-4 md:p-6">
                ...
                <div className="grid gap-4">
                    {modules.map(([modulo, modulePermissions]) => (<Card key={modulo}>
                            <CardHeader>
                ...
                <DialogContent>
                    ...
                    <form onSubmit={submit} className="space-y-4">
                        <div className="grid gap-2">
                            ...
                            <Input id="nombre" value={data.nombre} onChange={(event) => setData('nombre', event.target.value)} placeholder="roles:read"/>
                            <InputError message={errors.nombre}/>
                        </div>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="grid gap-2">
                                <Label htmlFor="modulo">Módulo</Label>
                                <Input id="modulo" value={data.modulo} onChange={(event) => setData('modulo', event.target.value)} placeholder="AccesoSeguridad"/>
                                <InputError message={errors.modulo}/>
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="accion">Acción</Label>
                                <Input id="accion" value={data.accion} onChange={(event) => setData('accion', event.target.value)} placeholder="LEER"/>
                                <InputError message={errors.accion}/>
                            </div>
                        </div>
                        <div className="grid gap-2">
                            <Label htmlFor="descripcion">Descripción</Label>
                            <Input id="descripcion" value={data.descripcion} onChange={(event) => setData('descripcion', event.target.value)}/>
            ...
            (selected ? !canUpdate : !canCreate)} className="bg-[#0D2B85] text-white hover:bg-[#0a2270]">
                                Guardar
                            </Button>
                        </div>
                    </form>
