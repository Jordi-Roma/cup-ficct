...
export default function DocentesTable({ docentes, canUpdate, canToggle, onEdit, }) {
    ...
    return (<div className="overflow-hidden rounded-md border">
            <div className="hidden overflow-x-auto xl:block">
                <table className="w-full min-w-[1000px] text-sm">
                    ...
                    <tbody className="divide-y">
                        {docentes.map((docente) => (<tr key={docente.id_docente}>
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex flex-wrap gap-1">
                                        ...
                                        {requirementBadge(docente.diplomado_educacion_superior, 'Diplomado')}
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                                    <Badge variant={docente.contratado
                ? 'default'
                : 'secondary'}>
                ...
                : 'secondary'}>
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex justify-end gap-2">
                                        {canUpdate && (<Button type="button" variant="outline" size="sm" onClick={() => onEdit(docente)}>
                                                <Edit2 className="size-4"/>
                                            </Button>)}
                                        {canToggle && (<Button type="button" variant="outline" size="sm" onClick={() => toggle(docente)}>
            ...
            <div className="grid gap-3 p-3 xl:hidden">
                {docentes.map((docente) => (<div key={docente.id_docente} className="space-y-3 rounded-md border p-4">
